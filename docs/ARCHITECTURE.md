...
1. Middleware classifies `skill_manage(create)` from supplied `content` and calls Hermes core at most once. Current Hermes `operations` batches are parsed before policy checks; supported single-operation batches retain the original envelope and call core once, while ambiguous or multi-operation batches are rejected before core because host rollback exceptions cannot be safely composed with registry ownership updates. The legacy flat envelope gets the same `write_file` shape contract: a missing or non-string `file_content` is rejected as `skill_publisher.operation_shape_invalid` before core. An explicitly empty `write_file` payload that would blank an existing non-empty skill file fails closed as `skill_publisher.empty_overwrite_blocked` in both envelopes before core runs; creating new files and writing empty content over already-empty files still passes. The entire classification/policy surface sits inside a fail-closed boundary: the host falls through to the core tool if a middleware callback raises before `next_call`, so parser or policy faults return a retryable JSON blocker instead.
